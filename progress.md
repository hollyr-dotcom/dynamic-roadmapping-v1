...
| `SidePanel.tsx` | View settings sidebar — accepts `onClose` and `fields` props; **header bar** (close button only on main view, back button on detail pages) with scroll-linked bottom border (200ms transition); "View settings" and "Fields" section headers inside scrollable content; **dynamic fields** — Fields list driven by `pageFields` from App.tsx (backlog fields or roadmap fields), icons mapped via `FIELD_TYPE_ICONS` (text→IconTextT, number→IconNumber, currency→IconDollarSignCurrency, avatars→IconOffice, status→IconTickCircle); full filter/layout/fields functionality with AI bar |
