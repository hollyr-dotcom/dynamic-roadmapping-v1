...
| `NameSpaceModal.tsx` | Post-creation modal — name input, import source pills (Jira/Tables/CSV with mutual exclusion), Insights enrichment toggle, entrance/exit animations (slide+fade), `animateOut` helper delays callback until exit completes |
...
- **Priority type:** `'now' | 'triage' | 'next' | 'later' | 'icebox'` (defined in shared `types.ts`)
- **Display labels:** New, Triage, Prioritized, Up next, Watching
- **Column order:** New → Triage → Prioritized → Up next → Watching
- **Table sort:** rows sorted by priority (column order); `IconSortDescending` indicator on Priority header (16px, `#7D8297`, 4px gap)
- **Priority column position:** between Title and Description in backlog fields
- **Backlog kanban** uses all 5 priorities; **Roadmap kanban** uses 3 (now/next/later)
- **Column colours** (bg / card border / tag bg / tag text):
  - New → cyan (`#e4f9ff` / `#b5ecff` / `#b5ecff` / `#003d54`)
  - Triage → coral (`#fff0f0` / `#ffc6c6` / `#ffc6c6` / `#600000`)
  - Prioritized → orange (`#ffeede` / `#ffc795` / `#ffc795` / `#5c3200`)
  - Up next → lilac (`#f3eeff` / `#d4bbff` / `#d4bbff` / `#2d0066`)
  - Watching → coal (`#f7f7f7` / `#dad8d8` / `#dad8d8` / `#222428`)
...
- **Name-after-creation flow** (`flow-gsf-nameaftercreation`) — moved "Name your space" modal from pre-creation (HomePage) to post-creation (App view); app opens immediately showing empty table behind the modal; `NameSpaceModal` extracted as standalone component (`NameSpaceModal.tsx`) with name input, import pills, enrichment toggle; `spaceName` state lifted to App.tsx and wired to TopNavBar breadcrumb, SpaceMenu sidebar header, and CanvasNavPanels; saved name updates everywhere; cancel returns to HomePage; Jira import modal follows after save if import source selected; HomePage dropdown simplified — only Roadmap is clickable, EPD WoW/Blueprint/Blank are visible and hoverable but non-functional; **empty input with placeholder** — input starts empty, random "Project [Planet]" name shown as placeholder only; Create button disabled (`isDisabled`) until user enters a name; space name hidden in nav breadcrumb until modal is submitted
- **Onboarding popover sequence** — two dark popovers (`#2B2D33`, 8px radius) with MDS-style ball+line anchor, 16px × close button; sequenced after import: (1) **Import popover** anchored to toolbar import button — "Import" / "Add more items from Jira, or upload a CSV."; appears 600ms after toast dismisses; dismisses on scroll or close; (2) **Share popover** anchored to Share button in TopNavBar — "Share [Space name]" / "Invite your team to collaborate."; appears 600ms after import popover dismissed; off-center anchor line points at button while card stays in viewport (`right: 12px`); custom fixed-position elements measured via refs (MDS Popover had positioning issues inside sticky toolbar); removed "Share with team" button from import success toast
- **Modal entrance/exit animations** — NameSpaceModal backdrop fades in (300ms) and panel slides up 12px + fades in (350ms, expo-out); on save/cancel, panel slides down 10px + fades out (200ms, ease-in) then callback fires; `isClosing` state + `animateOut` helper with 220ms delay before navigation; CSS keyframes: `center-overlay-in/out`, `center-panel-in/out`
- **Popover entrance animations** — two keyframes: `popover-tip-center-in` (centered, for import) and `popover-tip-in` (right-aligned, for share); both slide down 4px + fade in (300ms, expo-out)
- **Empty table state** — removed table icon above "No records yet" heading
- **Roadmap icon** — swapped `IconMap` → `IconRocket` in SpaceMenu sidebar
...
### Immediate — GSF empty state (`flow-gsf-emptystate-agents`)

**Done:**
- Locked empty state to "Disabled" variant (controls disabled, not hidden); removed A/B debug toggle UI
- Empty state renders for all view types (table, kanban, timeline) when space has no data — previously the timeline would render even without data
- **Page-aware icon** in blue square: `IconLightbulb` on Backlog, `IconRocket` on Roadmap
- **Page-aware heading copy**: Backlog → "Start adding ideas to your backlog"; Roadmap → "Start building your roadmap"
- Button order swapped: Import (primary) first, Add item (ghost) second; icons removed except `IconChevronDown` on Import; both buttons `size="medium"`
- Typography: heading bumped to 18px, description stays 14px
- Insights box: tighter gap between heading and description (`gap-0`); 16px left margin before toggle for breathing room; restructured to `justify-between` with icon+text on left, toggle on right
- **SpaceMenu**: Roadmap nav icon → `IconRocket` (was `IconMap`)
- **Empty state animation**: instant on first load (no `item-enter`), animated on page switch (remounts via `key={empty-${activePage}}`)
- **Share dialog entrance**: backdrop fades in (250ms), card slides up 14px with spring curve (350ms `cubic-bezier(0.16,1,0.3,1)`)
- **Empty state centering**: fills remaining viewport height below tabs via `flex-1` (was fixed `py-32`)
- **Agentic empty state** — replaced Insights toggle with "Add import agents" section: three agent cards (Insights, Jira, CSV) with `agent-exit` CSS keyframe (fade + scale + height collapse); `AgentAddedSnackbar` component auto-dismisses after 4s with "{Agent} agent added to {space name}"; cards use `flex gap-3` with locked `minHeight` on first removal so scroll area doesn't shrink; fixed bottom fade (`position: fixed`, 120px, `transparent → white`) with 160px bottom padding so last card scrolls clear
- **Robot icon in database title** — moved import agents button from toolbar to `DatabaseTitle`, between title and three-dots; robot SVG icon, hover-to-reveal same as three-dots, stays visible while dropdown/popover open; import dropdown (Jira, CSV) and onboarding popover repositioned here; toolbar import button removed

### Immediate — GSF empty state: improved discovery (`flow-gsf-emptystate-improvediscovery`)

**Done:**
- **Robot icon removed** from `DatabaseTitle`; **import button reinstated** in toolbar (rotated `IconSquareArrowIn`, between Sort and View Settings, Jira + CSV dropdown, ⌘I tooltip, active bg when open)
- **Empty state redesigned** — two sections with generous vertical spacing:
  - Section 1: inline text input — gray bg (`#f1f2f5`), no border; blue plus appears only when text is present and persists until cleared; placeholder "Write a roadmap idea" (backlog) / "Write a title" (roadmap); heading "Start with a new idea" / "Add a new roadmap item"
  - Section 2: import source cards — no border, borderless rows with hover bg; white circular plus button (gray border, black icon) turns blue on row hover; Backlog: Jira + CSV; Roadmap: "Move idea from backlog" (`IconLightbulb`) at top + Jira + CSV; heading "Add existing ideas" / "Add existing roadmap items"
...
### Immediate — Add to Board (`feature-add-to-board`)
...
### Recently completed
- **Backlog prioritization** (`feature-backlog-prioritization`) — renamed priority labels (Now→New, Next→Prioritized, Later→Up next, Icebox→Watching); reordered columns to New, Triage, Prioritized, Up next, Watching; table rows sorted by priority with `IconSortDescending` on header; Priority column repositioned between Title and Description; updated across 9 files (kanban, table, timeline, sidebars, sample data)
- **Toolbar: Import dropdown & Group button** (`feature-import-button`) — added Import icon button (rotated `IconSquareArrowIn`) between Sort and Settings with dropdown menu containing Jira (blue gradient logo) and CSV (`IconFileSpreadsheet`) options; centre-aligned dropdown panel; active `#F1F2F5` background on trigger while menu is open; `⌘+I` hotkey shown in tooltip; added Group icon button (`IconHorizontalBlocks`) before Filter with tooltip; all new buttons hoverable but not clickable; commented out Insights/AI sidekick toolbar entry point
- **Jira logo SVG replacement** (`fix-jiralogo`) — replaced all expired Figma MCP asset URLs and MDS `IconSocialJira` (black mono icon) with shared `JiraLogo` inline SVG component using the official blue gradient mark across 7 files: table cells, kanban cards, timeline bars, import dialogue, and all side panels
